...
namespace nlsat {
    ...
    bool interval_set_manager::complement_single(interval_set const *s) {
        if(s == nullptr || is_full(s)) {
            return false;
        }
        // check left most and right most
        if(!s->m_intervals[0].m_lower_inf || !s->m_intervals[s->m_num_intervals - 1].m_upper_inf) {
            return false;
        }
        bool exist = false;
        for(unsigned i = 0; i + 1 < s->m_num_intervals; i++) {
            interval const &left = s->m_intervals[i];
            interval const &right = s->m_intervals[i + 1];
